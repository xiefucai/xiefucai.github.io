			...
			$.extend(common.action,{
				'postForm':function(event,t){
					var form = t[0].form,
						...
						wmmEnabled = ['off','on'][1],
						//加密方式
						security = getFormValue('security') && srcData.wl0_security_choice || 'aes',
						//表单数据
						postData = {
							'wl_unit':0, // [2.4G,5G]
							'wl0_country_code':'US',
						...
						copyDataTo5G = function(data){
							...
							postData['wl1_chanspec'] = '149/80'; //5G模式下频道为149
							postData['wl1_bw_cap'] = 7;
							//5G无线模式为802.11a/n/ac
					...
					if (form['ssid_pasw'] && /^\s*$/.test(form['ssid_pasw'].value)){
						security = 'none';
					}
					...
					if (form['macvisible']){
						...
						common.protocol.getRemoteData('setWifiStrength',{'enable':(Boolean(+$(form['power']).attr('data-value'))).toString()},{'success':function(data){
							var code = +data.errorCode;
							if (code === 0){
								//setSelectorValue('power',enable);
							}
						}});
					}
				}
	...
	$.extend(window.common,{
		'init':function(){
			if (form['macvisible']){
				...
				common.protocol.getRemoteData('getWifiStrength',{},{'success':function(data){
					var code = +data.errorCode,
			...
			$.getJSON('/dataCenter.js',{
				...
				setCheckBoxValue('wifi5g_enable',!!json['wl1_radio']);
