...
if [ -n $isListened ];then
	...
	echo "用户可以在赚钱宝断网的时候 ，先别断电重启。盘拔一下，放一个<SN>.key进去(默认安装时已经生成)，再插到赚钱报上，这时候就会自动把网络诊断信息写在你的盘里，这时候 红灯会闪，红灯闪烁停止后  就表示诊断完毕了，可以把U盘里生成的<SN号>_<年月日>_<时分秒>.use文件发给我。比如ABPZ0F6214540_20151103_120132.use."
else

	echo "已经成功为赚钱宝($SN)安装了断网日志导出工具"
	echo "等赚钱宝断网的时候 ，先别断电重启。盘拔一下，放一个<SN>.key进去(默认安装时已经生成)，再插到赚钱报上，这时候就会自动把网络诊断信息写在你的盘里，这时候 红灯会闪，红灯闪烁停止后  就表示诊断完毕了，可以把U盘里生成的<SN号>_<年月日>_<时分秒>.use文件发给我。比如ABPZ0F6214540_20151103_120132.use."
fi